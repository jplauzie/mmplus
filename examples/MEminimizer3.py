...
outer_diameter = 30e-9
inner_diameter = 15e-9

circles = shapes.Circle(outer_diameter) - shapes.Circle(inner_diameter)
circles = circles.translate(outer_diameter / 2, outer_diameter / 2, 0)
...
#       2 = next 1 nm
#       ...
#       5 = innermost outer-edge band
#
#   6-10 : inner edge
#       6 = innermost 1 nm
#       ...
#       10 = outermost inner-edge band
# ------------------------------------------------------------------
...
# Coordinates of cell centers in mumaxplus indexing convention
ix = np.arange(nx)
iy = np.arange(ny)
...
# Bulk (region 0)
inside = (R >= inner_radius) & (R <= outer_radius)
regions[0][inside] = 0
...
for i in range(nbands):
    ...
    r_inner = outer_radius - (i + 1) * band_width
...
for i in range(nbands):
    ...
    r_inner = inner_radius + i * band_width
    r_outer = inner_radius + (i + 1) * band_width
...
plt.tight_layout()
plt.savefig("regions.png", dpi=200)

print("Defined regions:")
print(np.unique(regions))

print("Saved region map to regions.png")

plt.figure(figsize=(6, 6))
plt.imshow(regions[0], origin="lower", interpolation="nearest", cmap="tab20")
plt.colorbar(label="Region")
plt.title("Region map")
plt.xlabel("Cell x")
plt.ylabel("Cell y")
...
print("Creating magnet now")
print("World object:", world)
...
magnet.aex = 13e-12
# ------------------------------------------------------------------
# Region-dependent magnetic damping
# ------------------------------------------------------------------

bulk_alpha = 0.10

magnet.alpha = bulk_alpha
...
# Only assign values to regions that actually exist
defined_regions = np.unique(regions)

# Outer edge
for region, alpha in zip(range(1, 6), edge_alphas):
    if region in defined_regions:
        magnet.alpha.set_in_region(region, alpha)

# Inner edge
for region, alpha in zip(range(6, 11), reversed(edge_alphas)):
    if region in defined_regions:
        magnet.alpha.set_in_region(region, alpha)
...
magnet.C12 = 166e9
# ------------------------------------------------------------------
# Region-dependent elastic damping
# ------------------------------------------------------------------
...
# Outer edge
for region, eta in zip(range(1, 6), edge_etas):
    if region in defined_regions:
        magnet.eta.set_in_region(region, eta)

# Inner edge
for region, eta in zip(range(6, 11), reversed(edge_etas)):
    if region in defined_regions:
        magnet.eta.set_in_region(region, eta)
