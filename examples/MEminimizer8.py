...
# ----- Define Regions for Gradual Boundary Ramping -----
num_abs_cells = 32  # Width of absorbing region (in cells) on each side
max_alpha = 0.3     # Damping at the extreme outer edge
max_eta = 1e14      # Elastic damping at the extreme outer edge

# Initialize regions array using the shape mumaxplus expects: (nz, ny, nx)
regions = np.zeros((nz, ny, nx), dtype=int)

# 1. Assign unique region IDs to each cell layer along the X-edges
# Note: Since the array is [z, y, x], the x-coordinate is the LAST index
for i in range(num_abs_cells):
    regions[:, :, i] = i + 1

# Right edge cells get regions (num_abs_cells + 1) to (2 * num_abs_cells)
for i in range(num_abs_cells):
    idx = nx - num_abs_cells + i
    regions[:, :, idx] = num_abs_cells + 1 + i

# 2. Pass the regions array to Ferromagnet
magnet = Ferromagnet(world, grid, regions=regions)

# 3. Set default/center (Region 0) material parameters
magnet.alpha = 0.05
magnet.eta = 1e13

# 4. Loop through the region IDs to set the ramped values
ramp = np.linspace(1, 0, num_abs_cells) ** 2  # Quadratic profile

for i in range(num_abs_cells):
    # Calculate target values for layer i (0 is outermost edge, num_abs_cells-1 is innermost)
    alpha_val = 0.05 + (max_alpha - 0.05) * ramp[i]
    eta_val = 1e13 + (max_eta - 1e13) * ramp[i]

    # Region IDs
    left_region_id = i + 1
    right_region_id = 2 * num_abs_cells - i

    # Apply to Left Edge Region
    magnet.alpha.set_in_region(left_region_id, alpha_val)
    magnet.eta.set_in_region(left_region_id, eta_val)

    # Apply to Right Edge Region
    magnet.alpha.set_in_region(right_region_id, alpha_val)
    magnet.eta.set_in_region(right_region_id, eta_val)
...
magnet.external_body_force.remove_time_terms()
